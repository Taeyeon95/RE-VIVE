...
export function MigratePrompt({ onMigrate, onSkip }: Props) {
  ...
  return (
    <div className="bg-surface mx-auto flex min-h-svh max-w-md flex-col items-center justify-center gap-6 p-6 text-center">
      ...
      <h2 className="text-headline-lg-mobile text-primary">게스트로 쓰던 기록이 있어요</h2>
      <p className="text-body-md text-on-surface-variant">
        이 기기에 저장된 금연 기록을 계정으로 가져올까요? 가져오면 다른 기기에서도 이어서 볼 수 있어요.
      </p>
      <div className="flex w-full flex-col gap-3">
        <button type="button" onClick={handleMigrate} disabled={migrating} className={primaryButtonClass}>
